...
#define PTE_P                                                                  \
  0x001 // 是否指向一个存在的物理页框（page
        // frame）。如果该标志被设置，则表示该入口是有效的
#define PTE_W                                                                  \
  0x002 // 如果该标志被设置，则表示该页可以被写入，否则表示该页只能被读取
...
#define PTE_G                                                                  \
  0x100 // 该页框是否是全局页（global
        // page）。如果该标志被设置，则表示该页框是全局页，否则表示该页框只能被该进程使用。
...
struct Segdesc {
  ...
  unsigned sd_type : 4; // 表示段的类型，采用4个二进制位来表示。可取值包括代码段、数据段、TSS（任务状态段）等。(see
                        // STS_ constants)
  unsigned sd_s : 1; // 取值为0或1，0表示是系统段，1表示是应用程序段。
  unsigned
      sd_dpl : 2; // 描述符所属代码或数据是运行在内核空间还是用户空间，采用2个二进制位来表示。
  ...
  unsigned
      sd_db : 1; // 表示该段使用的是16位还是32位，取值为0或1，0表示使用16位，1表示使用32位。
  unsigned
      sd_g : 1; // 表示段的粒度，当该位为1时，段的大小限制被解释为4KB的倍数；否则，段的大小限制被解释为字节数。
