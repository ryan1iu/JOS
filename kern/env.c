...
static void region_alloc(struct Env *e, void *va, size_t len) {
  ...
  va = (pde_t *)ROUNDDOWN(va, PGSIZE);
  len = ROUNDUP(len, PGSIZE);
  uint32_t npage = len / PGSIZE;

  for (int i = 0; i < npage; i++) {
    page = page_alloc(0);
    page_insert(e->env_pgdir, page, va, PTE_P | PTE_U | PTE_W);
    va++;
  }
}
...
static void load_icode(struct Env *e, uint8_t *binary) {
  ...
  lcr3(PADDR(e->env_pgdir)); // change to user environment

  for (; ph < eph; ph++) {
    if (ph->p_type != ELF_PROG_LOAD)
      continue;
    if (ph->p_filesz >= ph->p_memsz)
      panic("file size great than mem size");
